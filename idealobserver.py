...
from particle import ParticleJCRP
...
#             for i, particle in enumerate(local_particles):
#                 s_t, c_o_t, c_r_t, j_t = particle.after_action(
#                     o_t, int(a_t), float(r_t)
#                 )
#                 vec_s[i] = s_t
#                 vec_c_o[i] = c_o_t
#                 vec_c_r[i] = c_r_t
...
#             (req_global_i,) = msg[
#                 1:
#             ]  # msg: ("get_states", requested_global_indices_list)

#             # Gather states for requested indices
#             global_to_local = {
#                 g: li for li, g in enumerate(particle_indices)
#             }  # particle_indices maps local->global
...
#             (new_states,) = msg[
#                 1:
#             ]  # msg: ("set_particles_from_states", new_states_list_in_worker_order)
...
#             (req_global_i,) = msg[
#                 1:
#             ]  # msg: ("get_particles", requested_global_indices_list)
...
#         if missing:
#             raise RuntimeError(
#                 f"Missing ancestor states for indices: {missing[:10]} ..."
#             )
...
#         p_s = torch.bincount(self.vec_s_t.to(torch.long), weights=w, minlength=2).to(
#             torch.float
#         )  # vec_s_t is [N] with entries 0/1
...
#         p_jump = (
#             w[self.vec_j_t.to(torch.long) == 1].sum().to(torch.float)
#         )  # vec_j_t is [N] with entries 0/1
...
def pf_worker_loop(
    ...
    # Create local particles for this worker
    local_particles = [ParticleJCRP(**hyp_params) for _ in particle_indices]
    local_logw = torch.zeros(len(local_particles), dtype=torch.float64)
    ...
    while True:
        ...
        if cmd == "before":
            ...
            # Each particle conducts inference on the latent state and contexts, and updates internal parameters
            vec_s = torch.empty(len(local_particles), dtype=torch.long)
            vec_c = torch.empty(len(local_particles), dtype=torch.long)
            vec_j = torch.empty(len(local_particles), dtype=torch.long)

            for i, particle in enumerate(local_particles):
                s_t, c_t, j_t = particle.after_action(o_t, int(a_t), float(r_t))
                vec_s[i] = s_t
                vec_c[i] = c_t
                vec_j[i] = j_t
                local_logw[i] = float(particle.log_weight)
            ...
            # Reconstitute particles from minimal states
            local_particles = [
                ParticleJCRP.from_state(hyp_params, st) for st in new_states
            ]
            # reset weights inside particles is done in your Ensemble.resample (you set log_weights=0)
            for p in local_particles:
                p.log_weight = 0.0
...
class IdealObsPFJCRP(SPMDWP, Subject):
    """
    Particle filtering implementation of the ideal observer. JCPR so there is only one context.
    """
    ...
    def __init__(self, N: int, n_workers: int, hyp_params: dict):
        ...
        self.log_weights = torch.zeros(N, dtype=torch.float64)
        self.vec_s_t = torch.zeros(N, dtype=torch.long)
        self.vec_c_t = torch.zeros(N, dtype=torch.long)
        self.vec_j_t = torch.zeros(N, dtype=torch.long)
    ...
    def after_action(self, o_t: torch.Tensor, a_t: int, r_t: float):
        ...
        for _, worker_id, logw_local, vec_s, vec_c, vec_j in msgs:
            shard = self.shards[worker_id]
            self.log_weights[shard] = logw_local
            self.vec_s_t[shard] = vec_s
            self.vec_c_t[shard] = vec_c
            self.vec_j_t[shard] = vec_j
