...
class ConjugateOptimalArm(ConjugateModel):
    ...
    def sample_post_dist(self):
        """
        Sample one reward probability p_a for each action from the
        posterior over the one-optimal-arm model.

        Returns:
            Tensor of shape [K], where exactly one arm has probability
            rho_c and all other arms have probability rho_i.
        """
        optimal_arm = D.Categorical(probs=self.beta_n).sample()

        p = self.beta0.new_full((self.K,), self.rho_i)
        p[optimal_arm] = self.rho_c
