...
# class SuffStatsWinningAction(SuffStats):
#     """
#     Sufficient statistics for the winning-action model.

#     possible[i] is True while action i is still a possible winning action.
#     Once an action receives reward 0, it is eliminated.
#     """

#     def __init__(self, K: int):
#         self.K = K
#         self.possible = torch.ones(K, dtype=torch.bool)

#     def update(self, a_t: int, r_t: int, confidence: float = 1.0):
#         """
#         Update sufficient statistics with one action/reward observation.

#         Because the reward is deterministic under this model, a reward of 0
#         eliminates the action as a possible winner, while a reward of 1 makes
#         it the only possible winner. Confidence is accepted for consistency
#         with other sufficient-statistics classes; any positive-confidence
#         observation has the same logical implication in this deterministic
#         model.
#         """
#         assert 0 <= a_t < self.K, f"action {a_t} out of range for {self.K} actions"
#         assert r_t in (0, 1), f"reward r_t must be 0 or 1, got {r_t}"
...
#         if r_t == 1:
#             self.possible.zero_()
#             self.possible[a_t] = True
#         else:
#             self.possible[a_t] = False

#     def as_tensor(self):
#         return self.possible.clone()

#     def to_state(self):
#         return {"possible": self.possible.clone()}

#     @classmethod
#     def from_state(cls, state: dict):
#         possible = state["possible"]
#         obj = cls(possible.shape[0])
#         obj.possible = possible.clone().to(torch.bool)
#         return obj


# class ConjugateWinningAction(ConjugateModel):
#     """
#     Conjugate model for a categorical winning-action variable.

#     There is exactly one winning action W in {0, ..., K-1}.

#     If action a_t is pulled, reward is 1 iff a_t is the winning action:
#         r_t = 1  => a_t is correct, so W == a_t
#         r_t = 0  => a_t is incorrect, so W != a_t

#     p_win[i] stores P(W == i | observations).
#     """

#     SUFFSTATS_CLS = SuffStatsWinningAction

#     def __init__(self, alpha0: torch.Tensor):
#         assert alpha0.dim() == 1, "alpha0 must be a 1D tensor"
#         assert torch.all(alpha0 >= 0), "alpha0 probabilities must be >= 0"
#         assert alpha0.sum() > 0, "alpha0 must have positive total probability"

#         self.alpha0 = alpha0.clone().to(torch.float)
#         self.alpha0 /= self.alpha0.sum()
#         self.K = self.alpha0.shape[0]

#         self.suffstats = self.SUFFSTATS_CLS(self.K)
#         self.reset_posterior()
...
#         Posterior parameters start at prior.
#         """
#         self.p_win = self.alpha0.clone()

#     def update(self, a_t: int, r_t: int, confidence: float = 1.0):
#         """
#         Update sufficient stats with (a_t, r_t) and refresh posterior.
#         """
#         assert 0 <= a_t < self.K, f"a_t must be in [0, {self.K})"
#         assert r_t in (0, 1), "r_t must be 0 or 1"

#         self.suffstats.update(a_t, r_t, confidence=confidence)
#         self._update_posterior()

#     def _update_posterior(self):
#         """
#         Reconstruct posterior probabilities from the sufficient statistics.
#         """
#         possible = self.suffstats.as_tensor()
#         self.p_win = self.alpha0.clone()
#         self.p_win[~possible] = 0.0

#         total = self.p_win.sum()
#         if total <= 0:
#             raise ValueError("No possible winning action remains.")
#         self.p_win /= total

#     def post_params(self):
#         return {"p_win": self.p_win}

#     def sample_post_dist(self):
#         """Sample the winning action from the posterior."""
#         return D.Categorical(probs=self.p_win).sample()

#     def _pred_dist_params(self):
#         """
#         Posterior predictive probabilities for the winning action.
#         """
#         return self.p_win

#     def pred_lh(self, a_t: int, r_t: int):
#         """
#         Predictive likelihood of reward r_t after pulling action a_t.

#         P(r_t = 1 | a_t) = P(W == a_t)
#         P(r_t = 0 | a_t) = 1 - P(W == a_t)
#         """
#         assert 0 <= a_t < self.K, f"a_t must be in [0, {self.K})"
#         assert r_t in (0, 1), "r_t must be 0 or 1"

#         p_win = self._pred_dist_params()[a_t]
#         return p_win.item() if r_t == 1 else (1.0 - p_win).item()


# class SuffStatsWinningAction(SuffStats):
#     """
#     Sufficient statistics for a noisy winning-action model.

#     The latent variable W is the unique winning action. The sufficient
#     statistic is the accumulated (possibly fractional) log-likelihood of
#     each possible value of W.
#     """

#     def __init__(self, K: int):
#         self.K = K
#         self.log_likelihood = torch.zeros(K)

#     def update(
#         self, a_t: int, r_t: int, confidence: float = 1.0, epsilon: float = 1e-3
#     ):
#         """Add fractional evidence from one action/reward observation."""
#         assert 0 <= a_t < self.K, f"action {a_t} out of range for {self.K} actions"
#         assert r_t in (0, 1), f"reward r_t must be 0 or 1, got {r_t}"
#         assert confidence >= 0.0, f"confidence must be >= 0, got {confidence}"
#         assert 0.0 < epsilon < 0.5, f"epsilon must be in (0, 0.5), got {epsilon}"

#         if confidence == 0.0:
#             return

#         if r_t == 1:
#             likelihood = torch.full((self.K,), epsilon)
#             likelihood[a_t] = 1.0 - epsilon
#         else:
#             likelihood = torch.full((self.K,), 1.0 - epsilon)
#             likelihood[a_t] = epsilon

#         self.log_likelihood += confidence * torch.log(likelihood)

#     def as_tensor(self):
#         return self.log_likelihood.clone()

#     def to_state(self):
#         return {"log_likelihood": self.log_likelihood.clone()}

#     @classmethod
#     def from_state(cls, state: dict):
#         log_likelihood = state["log_likelihood"]
#         obj = cls(log_likelihood.numel())
#         obj.log_likelihood = log_likelihood.clone().to(torch.float)
#         return obj


# class ConjugateWinningAction(ConjugateModel):
#     """
#     Bayesian model for a categorical winning-action variable with reward noise.

#     There is exactly one winning action W in {0, ..., K-1}.
#     With probability 1-epsilon, reward is correct relative to W; with
#     probability epsilon it is flipped. This keeps all hypotheses possible,
#     which allows confidence-weighted updates from latent-state/context
#     responsibilities.
#     """

#     SUFFSTATS_CLS = SuffStatsWinningAction

#     def __init__(self, alpha0: torch.Tensor, epsilon: float = 0.1):
#         assert alpha0.dim() == 1, "alpha0 must be a 1D tensor"
#         assert torch.all(alpha0 >= 0), "alpha0 probabilities must be >= 0"
#         assert alpha0.sum() > 0, "alpha0 must have positive total probability"
#         assert 0.0 < epsilon < 0.5, "epsilon must be in (0, 0.5)"
...
#     def update(self, a_t: int, r_t: int, confidence: float = 0.1):
#         """
#         Update sufficient stats with (a_t, r_t) and refresh posterior.
#         """
#         assert 0 <= a_t < self.K, f"a_t must be in [0, {self.K})"
...
#         self.suffstats.update(a_t, r_t, confidence=confidence, epsilon=self.epsilon)
#         self._update_posterior()
...
#         Reconstruct posterior probabilities from the sufficient statistics.
#         """
#         log_p = torch.log(self.alpha0) + self.suffstats.as_tensor()
#         self.p_win = torch.softmax(log_p, dim=0)
...
#         p_win = self._pred_dist_params()[a_t]
#         if r_t == 1:
#             return p_win * (1.0 - self.epsilon) + (1.0 - p_win) * self.epsilon
#         else:
#             return p_win * self.epsilon + (1.0 - p_win) * (1.0 - self.epsilon)
