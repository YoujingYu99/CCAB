...
class ExperimentalEnv:
    ...
    def repeat_trials(
        ...
        for i in range(N):
            ...
            self.trial_step(
                subject, c_o, c_r, experiment_history, subject_history, print_level
            )
...
class ExperimentalObs:
    ...
    def after_action(self, c_r, s_t, a_t):
        """
        Generate the reward and optimal action for the given context,
    ...
    def trial_step(
        ...
        assert isinstance(subject_history, SubjectHistory), (
            "subject_history must be an instance of the SubjectHistory class "
            "or its subclasses"
        )
        assert print_level in [
            0,
        ...
        #   Before action
        # -----------------
        # Pre-generated data supplies the true context, state and observation.
        ...
        #   After action
        # ----------------
        # As in ExperimentalEnv, reward is generated after action selection.
        # ExperimentalCRP has one pre-generated context, so it is used as
        # both the observation and reward context.
        r_t, opt_a_t = self.after_action(c_t, s_t, a_t)

        if print_level > 1:
            print(f", Action: {a_t}, Reward: {r_t}", end="")

        # Subject updates internal representations based on the selected
        # action and received reward
        subject.after_action(o_t, a_t, r_t)

        # Record keeping from the experimenter's perspective
        experiment_history.append(s_t, c_t, c_t, o_t, opt_a_t, a_t, r_t)

        # Record keeping from the subject's perspective, depends on the type
        # of subject
        if isinstance(subject, IdealObsPF) and isinstance(
            subject_history, IOPFSubjectHistory
        ):
